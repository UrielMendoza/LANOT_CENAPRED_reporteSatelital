...
def plotUbicaMuni(shpMuni,muni,nomMuni,x,y):
    ...
    gl.xlabel_style = {'size': 13, 'color': 'white'}
    gl.ylabel_style = {'size': 13, 'color': 'white','rotation':'vertical'}
    ...
    plt.text(shpMuni['geometry'][muni].centroid.x-0.05,shpMuni['geometry'][muni].centroid.y,nomMuni.upper(),fontsize=18,fontname = 'gadugi',color='white')
    
    plt.savefig('ubicacion_muni_'+str(x)+'_'+str(y)+'.png',transparent=True,dpi=300,bbox_inches='tight',pad_inches=0)

def plotUbicaEdo(shpEnti,enti,nomEnt,G16,SNPP,Sen2,x,y):
    ...
    ax.background_patch.set_facecolor('#666666')
    ...
    plt.fill([SNPP[0],SNPP[2],SNPP[2],SNPP[0],SNPP[0]],[SNPP[3],SNPP[3],SNPP[1],SNPP[1],SNPP[3]],'r',alpha=0.5,label='NOAA-20 / VIIRS')
    ...
    plt.savefig('ubicacion_edo_'+str(x)+'_'+str(y)+'.png',dpi=300,bbox_inches='tight',pad_inches=0)

def ubicaPuntosCalor(x,y): 

    shpEnti = gpd.read_file('./dest2018gw/dest2018gw.shp')
    shpMuni = gpd.read_file('./muni_2018gw/muni_2018gw.shp')
    ...
    plotUbicaEdo(shpEnti,enti,nomEnt,G16,SNPP,Sen2,x,y)
